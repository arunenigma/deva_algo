...
class DegreeOfMembership(object):
    ...
    def find_fuzzy_set(self, tf_idf_list, span, pivots):
        tf_idf_list_norm = {}
        if not len(tf_idf_list) == 0:
            max_tf_idf = max(tf_idf_list.values())
            for word, tf_idf in tf_idf_list.iteritems():
                if not max_tf_idf == 0.0:
                    tf_idf /= max_tf_idf
            ...
            for word, tf_idf in tf_idf_list_norm.iteritems():
                ...
                self.find_dom(fuzzy_sets)

    def find_dom(self, fuzzy_sets):
        dom_data = []
        dom_1 = self.triangular_function(fuzzy_sets[0][1], fuzzy_sets[0][3][0], fuzzy_sets[0][3][1],
                                         fuzzy_sets[0][3][2])
        dom_data.append([fuzzy_sets[0][0], fuzzy_sets[0][1], fuzzy_sets[0][2], dom_1])
        dom_2 = self.triangular_function(fuzzy_sets[0][1], fuzzy_sets[0][5][0], fuzzy_sets[0][5][1],
                                         fuzzy_sets[0][5][2])
        dom_data[0].extend([fuzzy_sets[0][4], dom_2])
        self.generate_dom_table(dom_data)

    @staticmethod
    def triangular_function(tf_idf, a, b, c):
        dom = 0.0
        if tf_idf <= a:
            dom += 0
        elif a <= tf_idf <= b:
    ...
    def generate_dom_table(self, dom_data):
