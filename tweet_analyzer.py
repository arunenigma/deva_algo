...
# tweet_analyzer.py generates input data for fuzzification
...
class LocationVector(object):
    def __init__(self, individual_tweet, words, tweet_bigrams, tweet_trigrams, tweet_fourgrams, tweet_fivegrams):
        self.tweet = individual_tweet
        self.tweet_words = words
        self.bi_grams = tweet_bigrams
        self.tri_grams = tweet_trigrams
        self.four_grams = tweet_fourgrams
        self.five_grams = tweet_fivegrams
        ...
        if branch.text is not None:
            ...
            if not branch.getchildren():
                ...
                for sentence in range(0, len(sentences)):
                    ...
                    for tweet_word in range(0, len(words)):
                        word_location = (("{0}[{1}][{2}]".format(index, sentence, tweet_word)), words[tweet_word])
                    ...
                    tweet_bigrams = bigrams(words)
                    if not len(tweet_bigrams) < 1:
                        for bi_gram in tweet_bigrams:
                            bi_gram = ' '.join(bi_gram)
                            self.bi_grams.append(bi_gram)

                    tweet_trigrams = trigrams(words)
                    if not len(tweet_trigrams) < 1:
                        for tri_gram in tweet_trigrams:
                            tri_gram = ' '.join(tri_gram)
                            self.tri_grams.append(tri_gram)

                    tweet_fourgrams = ngrams(words, 4)
                    if not len(tweet_fourgrams) < 1:
                        for four_gram in tweet_fourgrams:
                            four_gram = ' '.join(four_gram)
                            self.four_grams.append(four_gram)

                    tweet_fivegrams = ngrams(words, 5)
                    if not len(tweet_fivegrams) < 1:
                        for five_gram in tweet_fivegrams:
...
class WordTagger(HelperFunctions):
    ...
    def __init__(self, tweet_id, individual_tweet, words, tweet_bigrams, tweet_trigrams, tweet_fourgrams,
                 tweet_fivegrams, tweet_corpus, tweet_corpus_bigrams, tweet_corpus_trigrams, tweet_corpus_fourgrams,
                 tweet_corpus_fivegrams, count, word_count_list, bigram_count_list, trigram_count_list,
        ...
        self.corpus = tweet_corpus
        self.corpus_bigrams = tweet_corpus_bigrams
        self.corpus_trigrams = tweet_corpus_trigrams
        self.corpus_fourgrams = tweet_corpus_fourgrams
        self.corpus_fivegrams = tweet_corpus_fivegrams

        self.tweet_count = count
        self.tweet_word_count_list = word_count_list
        self.tweet_bi_gram_count_list = bigram_count_list
        self.tweet_tri_gram_count_list = trigram_count_list
        self.tweet_four_gram_count_list = fourgram_count_list
        self.tweet_five_gram_count_list = fivegram_count_list
    ...
    def generate_location_vector(self, branch, index):
        """
            generating location vector for every token (word) in tweet
        """
        if branch.text is not None:
            branch.text = branch.text.encode('ascii', 'ignore')
            ...
            if not branch.getchildren():
                ...
                for statement in range(0, len(statements)):
                    ...
                    for tweet_word in range(0, len(words)):
                        word_location = (("{0}[{1}][{2}]".format(index, statement, tweet_word)), words[tweet_word])
                        symbols = ",[]();:<>+=&+%!@#~?{}|"
                        whitespace = "                      "
                        replace = maketrans(symbols, whitespace)
                        tweet_word = word_location[1].translate(replace)
                    ...
                    tweet_bigrams = bigrams(words)
                    if not len(tweet_bigrams) < 1:
                        for i, bi_gram in enumerate(tweet_bigrams):
                    ...
                    tweet_trigrams = trigrams(words)
                    if not len(tweet_trigrams) < 1:
                        for i, tri_gram in enumerate(tweet_trigrams):
                    ...
                    tweet_fourgrams = ngrams(words, 4)
                    if not len(tweet_fourgrams) < 1:
                        for i, four_gram in enumerate(tweet_fourgrams):
    ...
    def tf(self):
        word_count = self.tweet_words.count(self.tweet_word)
        self.tf = word_count / float(len(self.tweet_words))
    ...
    def idf(self):
        word_occurrence = 0
        start, end = 0, 0
        for i in range(0, len(self.tweet_word_count_list)):
            end += self.tweet_word_count_list[i]
            if self.tweet_word in self.corpus[start:end]:
                word_occurrence += 1
            start = end + 1
        self.idf = log10(float(self.tweet_count + 1) / (word_occurrence + 1))

    def idf_bigram(self):
        word_occurrence = 0
        start, end = 0, 0
        for i in range(0, len(self.tweet_bi_gram_count_list)):
            end += self.tweet_bi_gram_count_list[i]
            if self.bi_gram in self.corpus_bigrams[start:end]:
                word_occurrence += 1
            start = end + 1
        self.idf_bigram = log10(float(self.tweet_count + 1) / (word_occurrence + 1))

    def idf_trigram(self):
        word_occurrence = 0
        start, end = 0, 0
        for i in range(0, len(self.tweet_tri_gram_count_list)):
            end += self.tweet_tri_gram_count_list[i]
            if self.tri_gram in self.corpus_trigrams[start:end]:
                word_occurrence += 1
            start = end + 1
        self.idf_trigram = log10(float(self.tweet_count + 1) / (word_occurrence + 1))

    def idf_fourgram(self):
        word_occurrence = 0
        start, end = 0, 0
        for i in range(0, len(self.tweet_four_gram_count_list)):
            end += self.tweet_four_gram_count_list[i]
            if self.four_gram in self.corpus_fourgrams[start:end]:
                word_occurrence += 1
            start = end + 1
        self.idf_fourgram = log10(float(self.tweet_count + 1) / (word_occurrence + 1))

    def idf_fivegram(self):
        word_occurrence = 0
        start, end = 0, 0
        for i in range(0, len(self.tweet_five_gram_count_list)):
            end += self.tweet_five_gram_count_list[i]
            if self.five_gram in self.corpus_fivegrams[start:end]:
                word_occurrence += 1
            start = end + 1
        self.idf_fivegram = log10(float(self.tweet_count + 1) / (word_occurrence + 1))

    def tf_idf(self):
        self.tfidf = self.tf * self.idf
        self.tf_idf_list[self.tfidf, self.word_location_index] = self.tweet_word
    ...
    def english_dict_match(self):
        """
            matching tweet word to English dictionaries
        """
        spell_checker_us = enchant.Dict('en_US')
        spell_checker_gb = enchant.Dict('en_GB')
        spell_checker_au = enchant.Dict('en_AU')

        if self.tweet_word.lower() in (corpus.stopwords.words('english')):
            pass

        if spell_checker_us.check(self.tweet_word.lower()) is True or spell_checker_gb.check(
                self.tweet_word.lower()) is True or spell_checker_au.check(self.tweet_word.lower()) is True:
            pass

        if HelperFunctions.all_lowercase(self.tweet_word) is None and spell_checker_us.check(
                self.tweet_word) is False:
            pass

    def number_match(self):
        if HelperFunctions.is_number(self.tweet_word):
            pass

    def smilie_match(self):
        if ':' in self.tweet_word or ')' in self.tweet_word or '(' in self.tweet_word:
            pass
    ...
    def pos_tagging_unigrams(self):
        pos = pos_tag([self.tweet_word])
        # NN or NNP --> Nouns
        if not self.tweet_word.lower() in (corpus.stopwords.words('english')) and not self.idf == 1.0 and \
                (pos[0][1] == 'NN' or pos[0][1] == 'NNP'):
            self.nouns_unigrams[self.tweet_word] = self.tfidf
        if not self.tweet_word.lower() in (corpus.stopwords.words('english')) and not self.idf == 1.0 and \
                        pos[0][1] == 'VB':
            self.verbs_unigrams[self.tweet_word] = self.tfidf
        if not self.tweet_word.lower() in (corpus.stopwords.words('english')) and not self.idf == 1.0:
            if re.search(r'\b[A-Z]+(?:\W*[A-Z]+)*\b', self.tweet_word):
                self.all_caps_unigrams[self.tweet_word] = self.tfidf
            if re.search(r'^[0-9]', self.tweet_word):
                self.numbers_unigrams[self.tweet_word] = self.tfidf
